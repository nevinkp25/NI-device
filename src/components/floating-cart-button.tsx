...
export function FloatingCartButton() {
  ...
  return (
    <>
      ...
      <div className={cn(
        "fixed bottom-0 left-1/2 -translate-x-1/2 w-full max-w-[420px] bg-background border-t shadow-[0_-10px_30px_rgba(0,0,0,0.15)] transition-all duration-300 z-50",
        isOpen ? "rounded-t-[2rem]" : ""
      )}>
        <Collapsible open={isOpen} onOpenChange={setIsOpen}>
          <CollapsibleContent className="p-4 space-y-4 max-h-[60vh] overflow-y-auto pt-2">
              <div className="flex justify-between items-center px-1 pt-2">
                <h3 className="font-bold text-xl text-slate-900">Your Selection</h3>
                <Button variant="ghost" size="sm" className="text-slate-400 font-bold uppercase text-[10px] tracking-widest" onClick={() => setIsOpen(false)}>
                    Close
                </Button>
              </div>
          ...
          <div className="flex items-center justify-between p-4 gap-4 bg-white">
            ...
            <Link href="/checkout" passHref className="flex-grow">
              <Button className="w-full h-12 bg-[#E54360] hover:bg-[#E54360]/90 text-white shadow-md rounded-2xl font-bold flex items-center justify-center gap-2">
                <span>Checkout</span>
